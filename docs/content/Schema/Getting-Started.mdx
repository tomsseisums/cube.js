...
calculate the paying users ratio, which is basically `payingCount / count`. Cube
makes it extremely easy to perform this kind of calculation. Let's add a new
measure to our cube called `payingPercentage`:
