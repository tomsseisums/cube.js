...
export class CubeStoreCacheDriver implements CacheDriverInterface {
  ...
  public withLock = (
    ...
    const rows = await this.driver.query(`CACHE SET NX TTL ${expiration} ? ?`, [key, '1']);
    if (rows && rows.length === 1 && rows[0]?.success === 'true') {
      ...
      try {
        ...
        if (freeAfter) {
          await this.driver.query(`CACHE REMOVE "${key}"`, []);
        }
      }
  ...
  public async get(key: string) {
    const rows = await this.driver.query(`CACHE GET "${key}"`, []);
  ...
  public async set(key: string, value, expiration) {
    ...
    await this.driver.query(`CACHE SET TTL ${expiration} ? ?`, [key, strValue]);
  ...
  public async keysStartingWith(prefix: string) {
    const rows = await this.driver.query(`CACHE KEYS "${prefix}"`, []);
